...
<section class="bg-white dark:bg-gray-900">
    <div class="max-w-2xl px-4 py-8 mx-auto lg:py-16">
        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{!! trans('subscriptions.update_record') !!}</h2>
        <form method="post" action="{{ route('subscriptions.update', $subscription->id) }}">
            @csrf
            @method('PUT')
            <div class="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6 sm:mb-5">
                <div class="sm:col-span-2">
                    <label for="small" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{!! trans('subscriptions.product_id') !!}</label>
                    <select id="client_id" name="client_id" class="block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <option value="{{ $client->id }}" {{ $client->id == $subscription->client_id ? 'selected' : ''}}>{{ $client->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="sm:col-span-2">
                    <label for="small" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{!! trans('subscriptions.client_id') !!}</label>
                    <select id="product_id" name="product_id" class="block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <option value="{{ $product->id }}" {{ $product->id == $subscription->product_id ? 'selected' : ''}}>{{ $product->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="w-full">
                    <label for="initial_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{!! trans('subscriptions.initial_date') !!}</label>
                ...
                <div class="w-full">
                    <label for="final_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{!! trans('subscriptions.final_date') !!}</label>
